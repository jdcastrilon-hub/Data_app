...
const STORAGE_KEY_COLUMNAS = 'monitorstock_vistavencimientos_columnas';

@Component({
  selector: 'vistavencimientos',
  standalone: true,
  ...
  templateUrl: './vista-vencimientos.component.html',
  styleUrl: './vista-vencimientos.component.scss'
})
export class VistaVencimientosComponent {
  ...
  columnasConfigurables: { clave: string, label: string }[] = [
    ...
    { clave: 'codigolote', label: 'Código Lote' },
    { clave: 'fecvencimiento', label: 'Fecha Vencimiento' },
    { clave: 'diasparavencer', label: 'Días para Vencer' },
  ...
  dataSource!: MatTableDataSource<LoteVencimiento>;
  ...
  ngOnInit() {
    ...
    this.dataSource = new MatTableDataSource<LoteVencimiento>([]);
  }
  ...
  exportarExcel(): void {
    ...
    this.service.exportarVencimientos(this.filtrosActuales || {}).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const enlace = document.createElement('a');
        enlace.href = url;
        enlace.download = 'vencimientos.xlsx';
        enlace.click();
        window.URL.revokeObjectURL(url);
        this.exportando = false;
      },
      error: (err) => {
        console.error('Error exportando vencimientos', err);
        this.exportando = false;
      }
    });
  }
