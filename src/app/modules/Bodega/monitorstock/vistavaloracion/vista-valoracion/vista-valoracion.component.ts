...
const STORAGE_KEY_COLUMNAS = 'monitorstock_vistavaloracion_columnas';

@Component({
  selector: 'vistavaloracion',
  standalone: true,
  ...
  templateUrl: './vista-valoracion.component.html',
  styleUrl: './vista-valoracion.component.scss'
})
export class VistaValoracionComponent {
  ...
  columnasConfigurables: { clave: string, label: string }[] = [
    ...
    { clave: 'costounitario', label: 'Costo Unitario' },
    { clave: 'valortotal', label: 'Valor Total' },
  ];
  columnasVisibles: Set<string> = new Set(this.columnasConfigurables.map(c => c.clave));

  displayedColumns: string[] = [];
  pageSizeOptions: number[] = [50, 100, 200, 300];
  dataSource!: MatTableDataSource<ValoracionDisponible>;
  ...
  ngOnInit() {
    ...
    this.dataSource = new MatTableDataSource<ValoracionDisponible>([]);
  }
  ...
  exportarExcel(): void {
    ...
    this.service.exportarValoracion(this.filtrosActuales || {}).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const enlace = document.createElement('a');
        enlace.href = url;
        enlace.download = 'valoracion.xlsx';
        enlace.click();
        window.URL.revokeObjectURL(url);
        this.exportando = false;
      },
      error: (err) => {
        console.error('Error exportando valoracion', err);
        this.exportando = false;
      }
    });
  }
